...
TypeORM's Postgres driver runs `CREATE EXTENSION IF NOT EXISTS "uuid-ossp"` **itself, on
connect**, as soon as any entity has a uuid column. It is not gated by `synchronize`. That is
DDL performed outside a migration, which defeats the whole point of the rule above: the
extension exists because something connected, not because a reviewed migration created it.

It also produced the wrong default. `@PrimaryGeneratedColumn('uuid')` emits
`DEFAULT uuid_generate_v4()` (uuid-ossp), while our `AddExtensions` migration installs
**pgcrypto**, whose function is `gen_random_uuid()`. A generated migration therefore depends on
an extension no migration creates — and fails on any database the ORM has not already touched.

**Both connection configs must set `uuidExtension: 'pgcrypto'`** — `src/database/data-source.ts`
(the CLI that generates and runs migrations) and `src/database/database.module.ts` (the app).
Setting it in one only is worse than neither, because the two would disagree about what the
default is.

After changing anything about uuid columns, check the extension list rather than assuming:

```sql
SELECT extname FROM pg_extension;
```

`uuid-ossp` appearing there is the symptom. Caught in the catalogue migration (Phase 4).
