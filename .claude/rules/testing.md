...
- **Request-scoped providers need `await module.resolve(X)`, not `module.get(X)`.**
  ...
  do. `module.get()` throws _"X is marked as a scoped provider"_ — this is the #1 confusing
...
**GraphQL returns HTTP 200 for every _execution-phase_ outcome, including auth failures.**

(The exception, verified against this server: a document that fails _parse or validation_ —
a typo'd field name, a syntax error — returns **400**, not 200. That's a malformed request,
not something application code produces, so it doesn't change any rule below; auth failures
are all execution-phase. See `graphql.md`'s error section for the full table.)
...
expect(printSchema(lexicographicSortSchema(schema)).trim()).toBe(
  (await readFile('schema.gql', 'utf8')).trim(),
);
```
...
await gql(
  app,
  `query { products(page:{limit:20}) { items { images { url } category { name } } } }`,
);
