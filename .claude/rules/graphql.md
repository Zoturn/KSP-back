...
## Errors: HTTP 200 for execution failures (but NOT for validation failures)
...
**The precise rule — verified against this server, not assumed:**

| Phase                                                                                 | Example                                                                  | HTTP status        |
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------------ |
| **Execution** — the document was valid, a resolver/guard failed                       | `UNAUTHENTICATED`, `FORBIDDEN`, `NOT_FOUND`, a service threw             | **200** + `errors` |
| **Parse / validation** — the document itself is malformed or doesn't match the schema | querying a field that doesn't exist, a syntax error, wrong argument type | **400** + `errors` |

Everything auth-related is execution-phase, so **auth tests still assert 200** and read
`errors[0].extensions.code` (see `testing.md`). The 400 case is a client-side mistake — a
typo'd query — not something application code produces, so it never needs handling in a
resolver. Don't branch on status codes either way; branch on `errors`.

Confirmed on this server: `{ fieldThatDoesNotExist }` → **400** with
`extensions.code: GRAPHQL_VALIDATION_FAILED`, while `{ apiStatus }` → 200. (Also visible in
that response: Apollo attaches `extensions.stacktrace` whenever `NODE_ENV !== 'production'` —
exactly the disclosure the Security section's `formatError` redaction closes in Phase 10.)
