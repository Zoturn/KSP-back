...
- **Node.js >= 24.9** — a hard floor: Jest only loads the ESM-shipping `@nestjs/*` packages
  natively on 24.9+, and only with `--experimental-vm-modules` (already in the `jest` npm
  script). Pinned via `engines` + `.nvmrc`. Run tests through `npm run test` / `test:e2e`,
  never `npx jest`. Full story in `.claude/rules/nestjs.md`.
...
> `@nestjs/apollo@13.1.0` forever _and_ mismatch Express. Also pin `graphql@^16.11.0` —
...
LEARNING/            # teaching notes (Docker, GraphQL, NestJS) — LOCAL ONLY, gitignored
```
